...
def _turbine_state(w_eff: float, rated_power: float) -> tuple[float, float]:
    ...
    match bisect.bisect_right(_REGIME_CUTS, w_eff):
        case 0:
            return 0.0, 0.0
        case 1:
            return rated_power * ((w_eff - 3) / 9) ** 2.5, 10 + (w_eff - 3) * 5
        case _:
            return rated_power, 55 + (w_eff - 12) * 0.2
...
def build_mapper_graph(X, filter1, filter2, n_bins=10, overlap=0.5, n_clusters=2):
    ...
    bins1 = [
        (f1_min + i * step1 * (1 - overlap), f1_min + (i + 1) * step1 * (1 + overlap))
        for i in range(n_bins)
    ]
    bins2 = [
        (f2_min + i * step2 * (1 - overlap), f2_min + (i + 1) * step2 * (1 + overlap))
        for i in range(n_bins)
    ]
    ...
    for i, (b1_low, b1_high) in enumerate(bins1):
        for j, (b2_low, b2_high) in enumerate(bins2):
            ...
            if len(points_in_bin) >= n_clusters:
                kmeans = KMeans(
                    n_clusters=min(n_clusters, len(points_in_bin)), random_state=42
                )
...
def classify_with_mapper(
    G, X_train, y_train, X_test, filter1_test, filter2_test, n_bins=10
):
    """
    ...
    node_centers = {}
    f1_train = np.array(
        [X_train[G.nodes[node]["indices"], 0].mean() for node in G.nodes()]
    )
    f2_train = np.array(
        [X_train[G.nodes[node]["indices"], 1].mean() for node in G.nodes()]
    )
...
def visualize_mapper_graph(G, y_labels, out_dir, plot: bool = False):
    ...
    if plot:
        fig, ax = plt.subplots(
            figsize=tuple(config.get("output", {}).get("figsize", [12, 10]))
        )
        ...
        ax.set_title(
            "Mapper Graph: Yaw Misalignment Detection", fontsize=14, fontweight="normal"
        )
        ax.axis("off")
        ...
        ax.legend(
            frameon=False, handles=legend_elements, loc="upper right", fontsize=11
        )
...
def main(plot: bool = False):
    ...
    if plot:
        ...
        ax.scatter(
            X_test[aligned_mask, 0],
            X_test[aligned_mask, 1],
            c="green",
            alpha=0.5,
            s=30,
            label="Aligned",
        ...
        ax.scatter(
            X_test[misaligned_mask, 0],
            X_test[misaligned_mask, 1],
            c="red",
            alpha=0.5,
            s=30,
            label="Misaligned",
        ...
        ax.set_title(
            "Filter Space: Aligned vs Misaligned Operation",
            fontsize=12,
            fontweight="normal",
        )
