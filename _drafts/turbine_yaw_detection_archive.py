...
def simulate_turbine_power_yaw(windspeed, wind_direction, turbine_yaw, rated_power=2.0):
    ...
    for i in range(len(windspeed)):
        ...
        if ws < cut_in or ws > cut_out:
            ...
            base_power = np.where(
                ws < rated_speed,
                rated_power * ((ws - cut_in) / (rated_speed - cut_in)) ** 3,
                rated_power,
            )
...
def create_yaw_scenarios(df, n_windows=120, window_size=288):
    ...
    logger.info(
        f"Created {sum(labels)} misaligned windows and {len(labels) - sum(labels)} aligned windows"
    )
...
def generate_visualizations(windows, labels, X, y, results, out_dir, plot: bool = False):
    ...
    if plot:
        ...
        plt.close()
        logger.info("  Saved: 3d_phase_space.png")
        ...
        plt.close()
        logger.info("  Saved: h2_distributions.png")
