...
def fetch_nrel_wind_data(lat=45.0, lon=-95.0, years=None):
    ...
    temperature = (
        5
        + 15 * np.cos(2 * np.pi * days / 365 - np.pi / 2)
        + np.random.normal(0, 4, n_records)
    )
    ...
    humidity = (
        65
        + 20 * np.cos(2 * np.pi * days / 365 - np.pi / 2)
...
def simulate_turbine_power_icing(
    windspeed, temperature, humidity, icing_severity=0, rated_power=2.0
):
    """
...
def create_icing_scenarios(df, n_windows=120, window_size=288):
    ...
    if len(icing_starts) < n_icing:
        logger.warning(
            f"  Warning: Only {len(icing_starts)} icing periods available, requested {n_icing}"
        )
    ...
    if len(no_icing_starts) < n_no_icing:
        logger.warning(
            f"  Warning: Only {len(no_icing_starts)} no-icing periods available, requested {n_no_icing}"
        )
...
def compute_multiparam_persistence_features(window_df):
    ...
    temp_norm = (temperature - temperature.min()) / (
        temperature.max() - temperature.min() + 1e-8
    )
...
def generate_visualizations(
    ...
    if plot:
        ...
        axes[0].scatter(
            no_icing_window["temperature"],
            no_icing_window["power"],
            alpha=0.5,
            s=20,
            color="#696969",
            edgecolors="#2b2b2b",
        ...
        plt.tight_layout()
        plt.savefig(
            out_dir / "multiparam_distributions.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
